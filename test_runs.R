...
for (i in 1:length(name_data)){
  ...
  for (k in 1:length(data)){
    ...
    type1 <- Blim_estim(stk, is.spasmodic=TRUE, doplot=TRUE)
    ...
    type2.6 <- Blim_estim(stk, doplot=TRUE, log=FALSE)
    ...
    type2 <- Blim_estim(stk, types=2, doplot=TRUE, log=FALSE)
    ...
    type3.6 <- Blim_estim(stk, types=3:6, doplot=TRUE)
